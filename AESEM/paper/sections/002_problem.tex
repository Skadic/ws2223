...
Note, that this method may not work for all algorithms. Examples of this will be discussed later. \info{Add reference to S-Algorithm chapter}
...
(see \info{Add reference to HS and S-Algorithm})
...
As we can see, the triangle inequality provides an algorithm with a simple way to determine whether a node can be added to a path.
...
However some approaches might even work as is on graphs with only the previous two restrictions (see \info{Add reference to HS chapter}) or they may be adapted to work with such graphs (see \info{Add reference to S-Algo Chapter}). 
