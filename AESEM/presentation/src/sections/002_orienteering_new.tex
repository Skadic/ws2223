...
\begin{frame}{The Orienteering Problem}
	...
	\begin{definition}[Orienteering Problem \cite{vansteenwegen_orienteering_2011}]
		\begin{itemize}
			\item<1-> Graph $G=(V = \{v_1, \dots, v_n\}, E)$
			\item<2-> Score function $s: V \rightarrow \mathbb{R}$
			\item<3-> Cost function $t: E \rightarrow \mathbb{R}_+$
			\item<4-> Cost Limit $T_{max}$
			\item<5->[$\Rightarrow$] Find path $P$ with maximal profit and cost less than $T_{max}$
		\end{itemize}
	\end{definition}
...
\begin{frame}{Complete Graphs}
	...
	\begin{columns}
		\begin{column}{0.58\textwidth}
			...
			\only<3->{ Almost unanimous in the literature \cite{vansteenwegen_orienteering_2011,laporte_selective_1990,santini_hazardous_2022,szwarc_novel_2022}}
		\end{column}
...
\begin{frame}{Triangle Inequality}
	...
	\note<3->[item]{Less often explicitly stated in literature}
	\note<3->[item]{
	...
	\begin{itemize}
		...
		\item<3-> Less often explicitly stated \cite{santini_hazardous_2022}
		\item<3-> More often implied by requiring a Euclidean metric
	\end{itemize}
\end{frame}
