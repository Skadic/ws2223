...
Raytracer::Raytracer(const std::string &filename) { read_scene(filename); }
...
void Raytracer::read_scene(const std::string &filename) {
    ...
    while (ifs && (ifs >> token) && (!ifs.eof())) {
        if (token[0] == '#') {
            ...
            Mesh *mesh = new Mesh((mode == "FLAT" ? Mesh::FLAT : Mesh::PHONG), fn);
 ...
 *      * Now, iterate a second time over your image and compare the color values of the each pixel with those of
 * its neighbors' pixels
...
void Raytracer::write_image(const std::string &filename) { image_.write_png(filename); }
...
vec3 Raytracer::trace(const Ray &ray, int depth) {
    ...
    if (material.mirror > 0 && depth <= max_depth_) {
        const Ray next_ray(point, mirror(-ray.direction_, normal));
        color = color * (1.0 - material.mirror) + trace(next_ray, depth + 1) * material.mirror;
    }
...
bool Raytracer::intersect_scene(const Ray &ray,
...
vec3 Raytracer::lighting(const vec3 &point, const vec3 &normal, const vec3 &view, const Material &material) {
    ...
    for (const auto light : lights_) {
        const auto l                   = normalize(light.position - point);
        const auto r                   = 2.0 * normal * dot(normal, l) - l;
        const auto in_angle_cos        = dot(normal, l);
        const auto view_diff_angle_cos = dot(r, view);
        if (in_angle_cos >= 0) {

            Ray shadow_ray(point, normalize(light.position - point));

            Material mat;
            vec3     pt, nm;
            double   dst;
            if (intersect_scene(shadow_ray, mat, pt, nm, dst) && dst < norm(light.position - point)) {
                continue;
            }
            color += light.color * material.diffuse * in_angle_cos;
            color += view_diff_angle_cos >= 0
                         ? light.color * material.specular * pow(view_diff_angle_cos, material.shininess)
                         : 0;
        }
    }
